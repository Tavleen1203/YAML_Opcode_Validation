...
class OpcodeValidator:
    ...
    def opcodes_to_dataframe(self):
        data = []
        for opcode, details in self.opcodes.items():
            args = ','.join(re.findall(r'{(\w+)}', details['format']))
            encoding = details['encoding']
            data.append([opcode, args, encoding])
        df = pd.DataFrame(data, columns=['Opcode', 'Arguments', 'Encoding'])
        return df
...
df = validator.opcodes_to_dataframe()
print(df)
